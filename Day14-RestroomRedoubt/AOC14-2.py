...
def count_robots_in_a_row(robots):
    max_count = 0
    for i in range(amount_of_rows):
        robots_in_row = []
        for robot in robots:
            if robot['endy'] == i:
                robots_in_row.append(robot)
        robots_in_row.sort(key=lambda x: x['endx'])
        max_row_count = 0
        continuous_count = 1
        start_pos = 0
        for robot in robots_in_row:
            if robot['endx'] == start_pos + 1:
                continuous_count += 1
            else:
                if continuous_count > max_row_count:
                    max_row_count = continuous_count
                continuous_count = 1
            start_pos = robot['endx']                
...
def print_map(robots):
    for i in range(amount_of_rows):
        ...
        for j in range(amount_of_columns):
            ...
            amount = 0
            for robot in robots:
                if robot['endx'] == j and robot['endy'] == i:
                    found = True
                    amount += 1
            line += str(amount) if found else '.'
        print(line)
...
def main():
    ...
    for i in range(10000):
        fast_move(robots,i)
        if count_robots_in_a_row(robots) > 20:
            print(f"Seconds: {i}")
            print_map(robots)
            return
